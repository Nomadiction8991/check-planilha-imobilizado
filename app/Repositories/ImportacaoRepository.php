...
class ImportacaoRepository extends BaseRepository
{
    ...
    public function criar(array $dados): int
    {
        ...
        $stmt->execute([
            ...
            ':administracao_id' => $dados['administracao_id'] ?? null,
    ...
    public function atualizar(int $id, array $dados): bool
    {
        ...
        if (isset($dados['total_linhas'])) {
            $sets[] = 'total_linhas = :total_linhas';
            $params[':total_linhas'] = $dados['total_linhas'];
        }
        ...
        if (isset($dados['iniciada_em'])) {
            $sets[] = 'iniciada_em = :iniciada_em';
            $params[':iniciada_em'] = $dados['iniciada_em'];
        }
    ...
    {
        $sql = "SELECT i.*, u.nome AS usuario_responsavel_nome, u.email AS usuario_responsavel_email
                      , a.descricao AS administracao_descricao
                FROM {$this->tabela} i
                INNER JOIN usuarios u ON u.id = i.usuario_id
                LEFT JOIN administracoes a ON a.id = i.administracao_id
                WHERE i.id = :id";
        $stmt = $this->conexao->prepare($sql);
        $stmt->execute([':id' => $id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);

        return $result ? $this->applyAdministrationLabel($result) : null;
    }

    public function buscarUltimaPorUsuario(int $usuarioId): ?array
    {
        $sql = "SELECT i.*, a.descricao AS administracao_descricao FROM {$this->tabela} i
                LEFT JOIN administracoes a ON a.id = i.administracao_id
     ...
     */
    private function applyAdministrationLabel(array $result): array
    {
        $administrationDescription = trim((string) ($result['administracao_descricao'] ?? ''));
        $result['administracao_label'] = $administrationDescription !== ''
