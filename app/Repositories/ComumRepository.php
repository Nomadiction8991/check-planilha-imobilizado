...
class ComumRepository extends BaseRepository
{
    ...
    protected array $colunas = ['codigo', 'cnpj', 'descricao', 'estado', 'cidade', 'estado_administracao', 'cidade_administracao', 'setor'];
    ...
    public function gerarCnpjUnico(string $cnpjBase, string $codigo, ?int $ignorarId = null): string
    {
    ...
    public function garantirPorCodigo(string $codigo, array $dados = []): int
    {
        $codigo = $this->normalizarCodigo($codigo);
        if ($codigo === '') {
        ...
        if ($existente) {
            ...
            if (!empty($dados)) {
                ...
                if (isset($dados['estado_administracao'])) {
                    $updates[] = "estado_administracao = :estado_administracao";
                    $params[':estado_administracao'] = $dados['estado_administracao'];
                }

                if (isset($dados['cidade_administracao'])) {
                    $updates[] = "cidade_administracao = :cidade_administracao";
                    $params[':cidade_administracao'] = $dados['cidade_administracao'];
                }
        ...
        $dadosInsert = [
            ...
            'cidade' => $dados['cidade'] ?? '',
            'estado_administracao' => $dados['estado_administracao'] ?? null,
            'cidade_administracao' => $dados['cidade_administracao'] ?? '',
    ...
    public function extrairCodigo(string $comumTexto): string
    {
    ...
    private function normalizarCodigo(string $codigo): string
    {
        ...
        if (preg_match('/^(\d{2})\D?(\d{4})$/', $codigo, $matches)) {
            return $matches[1] . '-' . $matches[2];
        }
