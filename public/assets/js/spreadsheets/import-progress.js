...
function iniciarProcessamento() {
    ...
    fetch('/spreadsheets/process-file', {
        ...
        .then(data => {
            if (data.success === false || data.erro) {
                mostrarErro(data.message || data.erro || 'Erro ao iniciar processamento');
                pararAtualizacao();
            }
        })
...
function atualizarProgresso() {
    fetch('/spreadsheets/api/progress?id=' + importacaoId)
        ...
        .then(data => {
            if (data.success === false || data.erro) {
                mostrarErro(data.message || data.erro || 'Erro ao consultar progresso');
                pararAtualizacao();
                return;
            }
