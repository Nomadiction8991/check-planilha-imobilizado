...
use App\Helpers\{PaginationHelper, ViewHelper};
...
<?php
// Construir campos base e avançados do filtro
$campos_filtro = [
    [
        'tipo'        => 'text',
        'name'        => 'filtro_complemento',
        'label'       => 'Pesquisar por Descrição',
        'value'       => $filtro_complemento ?? '',
        'placeholder' => 'Digite para buscar...',
    ],
];

// Gerar HTML dos filtros avançados
ob_start();
?>
<details class="mb-3">
...
<?php
$filtros_avancados = ob_get_clean();
?>

<?php
// Usar filter-card.php com campo extra para os filtros avançados
$filterCardOptions = [
    'titulo'       => 'FILTROS',
    'icone'        => 'bi-funnel',
    'campos'       => $campos_filtro,
    'total_label'  => ($total_registros ?? 0) . ' registros encontrados',
    'form_action'  => '',
    'hidden'       => ['comum_id' => $comum_id],
];

// Estender o partial para incluir filtros avançados
ob_start();
?>
                ...
                <div style="display:flex;gap:10px">
                    ...
                    <div style="flex:1;min-width:0">
                        <div style="font-weight:600;color:#000;margin-bottom:6px;line-height:1.3">
                            <?= htmlspecialchars(trim(($tipoPart ? $tipoPart . ' ' : '') . $descProd . $depPart)) ?>
                        </div>
                            ...
                            <div style="display:flex;gap:4px;flex-wrap:wrap">
                                <?php if (!empty($PRODUTO['codigo'])): ?>
                                    <span style="display:inline-block;padding:2px 7px;font-size:11px;font-weight:600;background:#f0f9ff;color:#0369a1;border:1px solid #06b6d4;border-radius:2px;font-family:Monaco,'Courier New',monospace">
                                        <?= ViewHelper::e(ViewHelper::formatarCodigoCurto($PRODUTO['codigo'] ?? '')) ?>
                                    </span>
                                <?php endif; ?>
...
$empty_msg = ($pesquisa_id || ($filtro_tipo_ben ?? '') || ($filtro_bem ?? '') || ($filtro_complemento ?? '') || ($filtro_dependencia ?? '') || ($filtro_STATUS ?? ''))
...
$confirmModalOptions = [
    ...
    'mensagem'    => 'Tem certeza que deseja excluir os produtos selecionados?',
