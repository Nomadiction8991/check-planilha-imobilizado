...
class ComumService
{
    ...
    public function criar(array $dados): int
    {
        ...
        $dados['codigo'] = $this->normalizarCodigo((string) $dados['codigo']);

        if ($dados['codigo'] === '') {
            throw new Exception('Código inválido.');
        }

        if ($this->comumRepository->buscarPorCodigo($dados['codigo'])) {
    ...
    public function atualizar(int $id, array $dados): bool
    {
        ...
        if (isset($dados['codigo']) && $dados['codigo'] !== (string) $comumExistente['codigo']) {
            if ($this->comumRepository->buscarPorCodigo($dados['codigo'])) {
                throw new Exception('Código já existe.');
            }
        }
        ...
        if (!empty($dados['cnpj'])) {
            ...
            $dados['cnpj'] = $this->comumRepository->gerarCnpjUnico(
                $dados['cnpj'],
                (string) ($dados['codigo'] ?? $comumExistente['codigo']),
                $id
            );
        }
