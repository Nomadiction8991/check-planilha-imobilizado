...
class ErrosImportacaoController extends BaseController
{
    ...
    private function erroPertenceAoUsuarioResponsavel(int $erroId): bool
    {
        ...
        $stmt = $this->conexao->prepare(
            'SELECT imp.usuario_id
             FROM import_erros ie
             INNER JOIN importacoes imp ON imp.id = ie.importacao_id
             WHERE ie.id = :id'
        );
        $stmt->execute([':id' => $erroId]);
        $usuarioIdResponsavel = (int) $stmt->fetchColumn();

        return $usuarioIdResponsavel > 0 && $usuarioIdResponsavel === $usuarioIdAtual;
    }
    ...
    public function listar(): void
    {
        ...
        $comumId      = (int) $this->query('comum_id', 0);
        $importacaoId = (int) $this->query('importacao_id', 0);
        $itensPorPagina = 30;
        ...
        if ($importacaoId > 0) {
            $stmtImp = $this->conexao->prepare(
                'SELECT i.arquivo_nome, i.iniciada_em, i.status,
                        u.nome AS usuario_responsavel_nome, u.email AS usuario_responsavel_email
                 FROM importacoes i
    ...
    public function marcarResolvido(): void
    {
        ...
        // SEGURANÇA: Validar tamanho da requisição (max 256KB para evitar DoS)
        ...
        $dados = json_decode(file_get_contents('php://input'), true, depth: 512);
        ...
        if (!$this->erroPertenceAoUsuarioResponsavel($erroId)) {
            http_response_code(403);
            echo json_encode(['erro' => 'Somente o responsável pela importação pode marcar este erro como resolvido.']);
            exit;
        }
