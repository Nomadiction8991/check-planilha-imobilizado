...
class UsuarioController extends BaseController
{
        ...
        try {
            ...
            ViewRenderer::render('users/create', [
                ...
                'old' => [
                    ...
                    'cpf' => $this->post('cpf', ''),
                    'rg' => $this->post('rg', ''),
                    'rg_igual_cpf' => $this->post('rg_igual_cpf', ''),
                    'telefone' => $this->post('telefone', ''),
                    'casado' => $this->post('casado', ''),
                    'nome_conjuge' => $this->post('nome_conjuge', ''),
                    'cpf_conjuge' => $this->post('cpf_conjuge', ''),
                    'rg_conjuge' => $this->post('rg_conjuge', ''),
                    'rg_conjuge_igual_cpf' => $this->post('rg_conjuge_igual_cpf', ''),
                    'telefone_conjuge' => $this->post('telefone_conjuge', ''),
                    'endereco_cep' => $this->post('endereco_cep', ''),
                    'endereco_logradouro' => $this->post('endereco_logradouro', ''),
                    'endereco_numero' => $this->post('endereco_numero', ''),
                    'endereco_complemento' => $this->post('endereco_complemento', ''),
                    'endereco_bairro' => $this->post('endereco_bairro', ''),
                    'endereco_cidade' => $this->post('endereco_cidade', ''),
                    'endereco_estado' => $this->post('endereco_estado', ''),
                ],
            ]);
        }
    }
    ...
    public function edit(): void
    {
        ...
        $this->renderizarFormularioEdicao($usuario);
    }
    ...
    public function update(int $id): void
    {
        try {
            ...
            $usuario = array_merge($usuario, $this->coletarDadosFormulario());
            $this->renderizarFormularioEdicao($usuario, $e->getMessage());
        }
    }
    ...
    private function redirecionarAposOperacao(string $queryExtra, string $mensagem = ''): void
    {
        ...
        $url = '/users';
        $query = http_build_query($retQ);
        if ($query !== '') {
            $url .= '?' . $query;
        }
        if ($queryExtra !== '') {
            $url .= ($query === '' ? '?' : '&') . $queryExtra;
        }
     ...
     * SEGURANÇA: Validação de Content-Type e Content-Length para evitar DoS
     */
    public function selecionarComum(): void
    {
        ...
        try {
            ...
            // SEGURANÇA: Validar tamanho da requisição (max 256KB para evitar DoS via JSON parsing)
            $contentLength = (int) ($_SERVER['CONTENT_LENGTH'] ?? 0);
            if ($contentLength > 262144) { // 256KB
                http_response_code(413);
                echo json_encode(['success' => false, 'message' => 'Requisição muito grande']);
                exit;
            }

            if ($contentLength === 0) {
                http_response_code(400);
                echo json_encode(['success' => false, 'message' => 'Corpo da requisição vazio']);
                exit;
            }

            $input = json_decode(file_get_contents('php://input'), true, depth: 512);
            ...
            $comumRepository = new ComumRepository($this->conexao);
            $comum = $comumRepository->buscarPorId($comumId);
            if (!$comum) {
                http_response_code(404);
                echo json_encode(['success' => false, 'message' => 'Comum não encontrada']);
                exit;
            }
            ...
            echo json_encode(['success' => false, 'message' => 'Erro ao selecionar a comunidade.']);
        }
        exit;
    }
